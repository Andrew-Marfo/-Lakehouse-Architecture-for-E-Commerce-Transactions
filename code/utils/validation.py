...
def validate_dataframe(df: DataFrame, schema: StructType, primary_key: str, required_columns: list,
                       rejected_path: str) -> DataFrame:
    """
    ...
    try:
        ...
        schema_mismatch_df = df.filter(
            reduce(lambda x, y: x | y, [col(field.name).isNull() for field in schema.fields
